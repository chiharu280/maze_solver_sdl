...
- `generate_perfect_maze()` 接受奇数宽高并写入调用方提供的网格，每行以 `\0` 结尾；生成结果具有封闭边界、唯一 `S`/`E`，且所有通路连通无环。`3 x 3` 会因 `S`/`E` 重叠而被拒绝。
- 相同尺寸与相同种子必须得到相同迷宫，便于复现测试；生成器使用自己的伪随机状态，不得修改进程级 `rand()` 状态或迷宫全局变量。
